...
class PrioritizedExperienceReplayMemory:
    ...
    def __init__(self, config: Config):
        ...
        self.blank_trans = Transition(0, torch.zeros(3, self.config.frame_height, self.config.frame_width, dtype=torch.uint8), None, 0, False, 0)
    ...
    def _get_transition(self, idx):
        ...
        for t in range(self.config.frame_history - 2, -1, -1):  # e.g. 2 1 0 with history 4
            if transition[t + 1].timestep == 0:
                ...
                index = idx + (-self.config.frame_history + t + 1)
    ...
    def _get_sample_from_segment(self, segment, i):
        ...
        while not valid:
            ...
            if (self.transitions.index - idx) % self.config.memory_capacity > self.config.multi_step and (idx - self.transitions.index) % self.config.memory_capacity >= self.config.frame_history and prob != 0:
        ...
        state = (torch.stack([trans.state for trans in transition[:self.config.frame_history]]).to(dtype=torch.float32, device=self.config.device).div_(127.5)-1.0)[0]

        test = [trans.state for trans in transition[self.config.multi_step:self.config.multi_step + self.config.frame_history]]
        next_state = (torch.stack(test).to(dtype=torch.float32, device=self.config.device).div_(127.5)-1.0)[0]
    ...
    def __next__(self):
        ...
        for t in reversed(range(self.config.frame_history - 1)):
            if prev_timestep == 0:
                ...
                prev_timestep -= 1
        state = (torch.stack(state_stack, 0).to(dtype=torch.float32, device=self.config.device).div_(127.5)-1.0)[0]  # Agent will turn into batch
        self.current_idx += 1
        return state
